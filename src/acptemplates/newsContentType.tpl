			...
			<li>
				<div>
					<div class="containerHeadline">
						<h3>
							<label{if $categoryItem->getDescription()} class="jsTooltip" title="{$categoryItem->getDescription()}"{/if}> <input type="checkbox" name="contentData[categoryIDs][]" value="{@$categoryItem->categoryID}"
							                                                                                                                    class="jsCategory"{if $contentData[categoryIDs]|isset && $categoryItem->categoryID|in_array:$contentData[categoryIDs]} checked="checked"{/if} />
						...
						<ol>
							{foreach from=$categoryItem item=subCategoryItem}
								{if $subCategoryItem->isAccessible()}
									<li>
										<label{if $subCategoryItem->getDescription()} class="jsTooltip" title="{$subCategoryItem->getDescription()}"{/if}> <input type="checkbox" name="contentData[categoryIDs][]" value="{@$subCategoryItem->categoryID}"
										                                                                                                                          class="jsChildCategory"{if $contentData[categoryIDs]|isset && $subCategoryItem->categoryID|in_array:$contentData[categoryIDs]} checked="checked"{/if} />
...
<dl>
	<dt>
		<label for="contentData[type]">{lang}cms.acp.content.type.de.codequake.cms.content.type.news.type{/lang}</label>
	</dt>
	<dd>
...
<dl>
	<dt>
		<label for="contentData[limit]">{lang}cms.acp.content.type.de.codequake.cms.content.type.news.limit{/lang}</label>
	</dt>
	<dd>
