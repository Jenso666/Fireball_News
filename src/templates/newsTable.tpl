            ...
            {foreach from=$objects item=news}
                <tr id="news{@$news->newsID}" class="cmsNews jsNewsRow jsClipboardObject{if $news->isNew()} new{/if}" data-news-id="{@$news->newsID}" data-element-id="{@$news->newsID}">
                    ...
                    <td class="columnText columnSubject">
                        ...
                        <small>
                            {if $news->userID}
                                <a href="{link controller='User' object=$news->getUserProfile()->getDecoratedObject()}{/link}" class="userLink" data-user-id="{@$news->userID}">{$news->username}</a>{else}{$news->username}{/if}
                            - {@$news->time|time}
                        </small>
                        ...
                        {if $news->getCategories()|count}
                            <ul class="dataList">
                                {foreach from=$news->getCategories() item=category}
                                    <li>
                                        <a href="{link application='cms' controller='NewsCategoryList' object=$category}{/link}" class="jsTooltip" title="{lang}cms.news.categorizedNews{/lang}">{$category->getTitle()}</a>
                                    </li>
                                {/foreach}
                            </ul>
                        {/if}
