	...
	<table class="table">
		...
		<tbody>
			{foreach from=$objects item=news}
				<tr id="news{@$news->newsID}" class="cmsNews jsNewsRow jsClipboardObject{if $news->isNew()} new{/if}{if $news->isDisabled} messageDisabled{/if}{if $news->isDeleted} messageDeleted{/if}" data-news-id="{@$news->newsID}" data-element-id="{@$news->newsID}">
					{if $__wcf->session->getPermission('mod.fireball.news.canModerateNews')}
						<td class="columnMark jsOnly">
							<label><input type="checkbox" class="jsClipboardItem" data-object-id="{@$news->newsID}" /></label>
						</td>
						<td class="columnIcon jsOnly">
					...
					<td class="columnText columnSubject">
						...
						<aside class="statusDisplay">
							<ul class="statusIcons">
								{if $news->attachments}
									<li>
										<span class="icon icon16 icon-paper-clip jsTooltip" title="{lang}cms.news.attachments{/lang}"></span>
									</li>
								{/if}
						...
						<small>
							{if $news->userID}
								<a href="{link controller='User' object=$news->getUserProfile()->getDecoratedObject()}{/link}" class="userLink" data-user-id="{@$news->userID}">{$news->username}</a>
							{else}
								{$news->username}
							{/if}
							- {@$news->time|time}
						...
						{if $news->getCategories()|count}
							<ul class="dataList">
								{foreach from=$news->getCategories() item=category}
									<li>
										<a href="{link application='cms' controller='NewsOverview' object=$category}{/link}" class="jsTooltip" title="{lang}cms.news.categorizedNews{/lang}">{$category->getTitle()}</a>
									</li>
								{/foreach}
							</ul>
						{/if}
