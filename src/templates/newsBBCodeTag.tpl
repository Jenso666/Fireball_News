<div class="container containerPadding marginTop">
	...
	<div class="box96">
		...
		<div class="containerHeadline">
			<h3>
				<a href="{$_news->getLink()}" data-news-id="{$_news->newsID}" class="newsLink cmsNewsLink"
				   title="{$_news->getTitle()}">{$_news->getTitle()}</a>
			</h3>
			...
			<p>
			    ...
			    <span class="username">
				    {if $_news->userID != 0}
					    <a class="userLink" data-user-id="{$_news->userID}"
					       href="{link controller='User' object=$_news->getUserProfile()}{/link}">
                            {$_news->username}
                        </a>
				    {else}
					    {$_news->username}
