	...
	<fieldset>
		...
		<div class="box32">
			<div class="userAvatar">
				<a class="framed userLink" data-user-id="{$news->getUserProfile()->userID}"
				   href="{link controller='User' object=$news->getUserProfile()}{/link}">{@$news->getUserProfile()->getAvatar()->getImageTag(24)}</a>
			</div>
			<div class="userDetails">
				<div class="containerHeadline">
					{if $news->userID != 0}
						<h3>
							<a class="userLink" rel="author" data-user-id="{$news->getUserProfile()->userID}"
							   href="{link controller='User' object=$news->getUserProfile()}{/link}">{$news->getUserProfile()->username}</a>
						</h3>
					{else}
						{$news->username}
					{/if}
		...
		<fieldset>
			...
			<ul>
				{foreach from=$news->getCategories() item=category}
					<li>
						<a href="{link application='cms' controller='NewsList' object=$category}{/link}"
						   class="jsTooltip" title="{lang}cms.news.categorizedNews{/lang}">{$category->getTitle()}</a>
					</li>
				{/foreach}
			</ul>
		</fieldset>
		...
		<fieldset>
			...
			<ul class="tagList">
				{foreach from=$tags item=tag}
					<li>
						<a href="{link controller='Tagged' object=$tag}objectType=de.codequake.cms.news{/link}"
						   class="badge tag jsTooltip"
						   title="{lang}wcf.tagging.taggedObjects.de.codequake.cms.news{/lang}">{$tag->name}</a></li>
				{/foreach}
		</fieldset>
	{/if}
...
{if ($news->isDisabled && $news->canSeeDelayed()) || !$news->isDisabled}
						...
						<div>
							{if CMS_NEWS_NEWS_IMAGES_ATTACHED && $news->imageID != 0 && CMS_NEWS_NEWS_IMAGES_FULLSCREEN}
								<div class="fullScreenPicture"
								     style="background-image: url({@$news->getImage()->getLink()});"></div>
							{/if}
							<header class="messageHeader">
								<div class="messageHeadline">
									...
									<p>
											<span class="username">
												{if $news->userID != 0}
													<a class="userLink" data-user-id="{$news->userID}"
													   href="{link controller='User' object=$news->getUserProfile()}{/link}">
                                                        {$news->username}
                                                    </a>
												{else}
													{$news->username}
										...
										<a class="permalink"
										   href="{link controller='News' object=$news application='cms'}{/link}">
											{@$news->time|time}
										</a>
									</p>
								</div>
							...
							<div class="messageBody">
								...
								{if CMS_NEWS_NEWS_IMAGES_ATTACHED && $news->imageID != 0 && !CMS_NEWS_NEWS_IMAGES_FULLSCREEN}
									<div class="newsBox128">
										<div class="framed">
											<img src="{@$news->getImage()->getLink()}"
											     alt="{$news->getImage()->getTitle()}" style="width: 128px"/>
										</div>
										{if $news->teaser!= ''}
											<div class="newsTeaser">
												{$news->teaser}
								...
								<footer class="messageOptions">
									<nav class="buttonGroupNavigation jsMobileNavigation">
										<ul class="smallButtons buttonGroup">
											{if $news->canModerate()}
												<li>
													<a href="{link controller='NewsEdit' application='cms' object=$news}{/link}"
													   class="button jsMessageEditButton"
													   title="{lang}wcf.global.button.edit{/lang}">
											...
											{if $news->canModerate()}
												<li class="jsOnly">
													<a class="button jsDeleteButton jsTooltip pointer"
													   title="{lang}wcf.global.button.delete{/lang}"
													   data-object-id="{@$news->newsID}"
													   data-confirm-message="{lang}cms.news.delete.sure{/lang}">
											...
											<li class="toTopLink">
												<a href="{@$anchor}" title="{lang}wcf.global.scrollUp{/lang}"
	...
	<div class="contentNavigation">
		<nav>
			<ul>
				<li>
					<a href="{link application='cms' controller='News' object=$news appendSession=false}{/link}"
					   class="button jsButtonShare jsOnly" data-link-title="{$news->subject}">
