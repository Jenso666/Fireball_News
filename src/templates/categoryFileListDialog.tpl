<div class="tabularBox tabularBoxTitle marginTop" data-category-id="{@$category->categoryID}">
	...
	<table class="table">
		...
		{foreach from=$fileList item=file}
			<tr data-file-id="{@$file->fileID}" data-file-title="{$file->getTitle()}"
			    data-file-formatted-size="{@$file->filesize|filesize}">
				<td class="columnMark"></td>
				<td class="columnID columnFileID">{@$file->fileID}</td>
				<td class="columnIcon">{@$file->getIconTag()}</td>
				<td class="columnTitle columnFile"><a class="cmsFileLink"
				                                      data-file-id="{@$file->fileID}">{$file->getTitle()}</a></td>
