<fieldset>
	...
	<ul>
		...
		{if $registrationIpAddress|isset}
			<li>
				{lang}cms.news.ipAddress.registration{/lang} {$registrationIpAddress[ipAddress]}
				({$registrationIpAddress[hostname]})
			</li>
		{/if}
	</ul>
</fieldset>
	...
	<fieldset>
		...
		<ul>
			...
			{foreach from=$otherUsers item=user}
				{if $user[userID]}
					<li>
						<a href="{link controller='User' id=$user[userID] title=$user[username]}{/link}"
						   class="userLink" data-user-id="{@$user[userID]}">{$user[username]}</a>
					</li>
				{else}
