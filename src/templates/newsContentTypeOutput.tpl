        ...
        <div class="box128">
            <div class="newsImage">
                <a href="{link controller='News' object=$news application='cms'}{/link}">
                    <img src="{if $news->imageID != 0}{$news->getImage()->getLink()}{else}{$news->getUserProfile()->getAvatar()->getURL()}{/if}" alt="{$news->getTitle()}" style="max-width: 120px;" />
                </a>
            </div>
...
{else if $type == 'simple2'}
    {foreach from=$objects item=news}
        <div class="box48 simpleNews">
            <div class="newsImage">
                <a href="{link controller='News' object=$news application='cms'}{/link}">
                    <img src="{if $news->imageID != 0}{$news->getImage()->getLink()}{else}{$news->getUserProfile()->getAvatar()->getURL()}{/if}" alt="{$news->getTitle()}" style="max-width: 40px;" />
                </a>
            </div>
            <div class="containerHeadline">
                <h3>
