...
<form id="messageContainer" class="jsFormGuard" method="post"
      action="{if $action == 'add'}{link controller='NewsAdd' application='cms'}{/link}{else}{link controller='NewsEdit' application='cms' id=$newsID}{/link}{/if}">
	<div class="container containerPadding marginTop">
									...
									<h3>
										<label{if $categoryItem->getDescription()} class="jsTooltip" title="{$categoryItem->getDescription()}"{/if}>
											<input type="checkbox" name="categoryIDs[]"
											       value="{@$categoryItem->categoryID}" class="jsCategory"
												...
												<li>
													<label{if $subCategoryItem->getDescription()} class="jsTooltip" title="{$subCategoryItem->getDescription()}"{/if}>
														<input type="checkbox" name="categoryIDs[]"
														       ...
														       class="jsChildCategory"
														       {if $subCategoryItem->categoryID|in_array:$categoryIDs}checked="checked" {/if}/>
		...
		<fieldset>
			...
			<dl{if $errorField == 'subject'} class="formError"{/if}>
				...
				<dd>
					<input type="text" id="subject" name="subject" value="{$subject}" required="required"
					       maxlength="255" class="long"/>
		...
		<fieldset class="jsOnly">
			...
			<dl{if $errorField == 'time'} class="formError"{/if}>
				...
				<dd>
					<input data-ignore-timezone="1" type="datetime" class="medium" id="time" name="time"
					       value="{$time}"/>
				</dd>
			</dl>
		</fieldset>
