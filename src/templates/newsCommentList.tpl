...
<div class="container containerList marginTop">
	...
	{if $commentList|count || $commentCanAdd}
		<ul id="newsCommentList" class="commentList containerList" data-can-add="true" data-object-id="{@$news->newsID}"
		    data-object-type-id="{@$commentObjectTypeID}" data-comments="{@$commentList->countObjects()}"
		    data-last-comment-time="{@$lastCommentTime}">
			{include file='commentList'}
		</ul>
	{else}
		{hascontent}
			<ul id="newsCommentList" class="commentList containerList" data-can-add="false"
			    data-object-id="{@$news->newsID}" data-object-type-id="{@$commentObjectTypeID}"
